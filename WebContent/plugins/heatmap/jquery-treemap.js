(function($) {
	...
	TreeMap.SIDE_MARGIN = 0.1;
	TreeMap.TOP_MARGIN = 0;
	...
	TreeMap.prototype.paint = function(nodeList) {
		...
		for (var i = 0; i < nodeList.length; i++) {
			...
			var boxId = i;
			var value = node.notSold;
			var name = node.label;
			var name = name.replace(' ', '&nbsp;');
			var name = name.replace(' ', '&nbsp;');
			var name = name.replace(' ', '&nbsp;');
			var name = name.replace(' ', '&nbsp;');
			var $box = $('<div id=' + boxId + '  ' + 'title=' + value
					+ '&nbsp;are' + '&nbsp;not' + '&nbsp;sold'
					+ '&nbsp;for&nbsp;' + name + '&nbsp;territory ></div>');
			...
			var $content = $("<div>" + node.label
					+ "<br /><b>Total Stores:</b>" + node.totalStore
					+ "<br /><b>Total Distribution Points:</b>"
					+ node.totalDistributionPoint + "<br /><b>Sold:</b>"
					+ node.sold + "<br /><b>Not Sold:</b>" + node.notSold
					+ "<br />" + node.value1 + "% products are not sold"
					+ "</div>");
			...
			if (this.centerLabelVertically) {
				$content.css('margin-top', (parseInt($box.height()) / 2)
						- (parseInt($content.height()) / 2) + 'px');
			}
		}
		this.ready();
	};
	...
	TreeMap.prototype.fitLabelFontSize = function($content, node) {
		var nodeBounds = node.bounds;
		while ($content.height() + TreeMap.TOP_MARGIN > nodeBounds.height
				|| $content.width() + TreeMap.SIDE_MARGIN > nodeBounds.width) {
	...
	TreeMap.prototype.divideDisplayArea = function(nodeList, destRectangle) {
		...
		var leftSum = this.sumValues(halves.left), rightSum = this
				.sumValues(halves.right), totalSum = leftSum + rightSum;

		if (destRectangle.isWide()) {
			var midPoint = Math.round((leftSum * destRectangle.width)
					/ totalSum);
			this.divideDisplayArea(halves.left, new Rectangle(destRectangle.x,
					destRectangle.y, midPoint, destRectangle.height,
					this.itemMargin));
			this.divideDisplayArea(halves.right, new Rectangle(destRectangle.x
					+ midPoint, destRectangle.y,
					destRectangle.width - midPoint, destRectangle.height,
					this.itemMargin));
		} else {
			var midPoint = Math.round((leftSum * destRectangle.height)
					/ totalSum);
			this.divideDisplayArea(halves.left, new Rectangle(destRectangle.x,
					destRectangle.y, destRectangle.width, midPoint,
					this.itemMargin));
			this.divideDisplayArea(halves.right, new Rectangle(destRectangle.x,
					destRectangle.y + midPoint, destRectangle.width,
					destRectangle.height - midPoint, this.itemMargin));
		}
	};
	...
	TreeMap.prototype.splitFairly = function(nodeList) {
		...
		for (var midPoint = 0; midPoint < length; midPoint++) {
			if (midPoint > 0
					&& (accValue + nodeList[midPoint].value > halfValue))
				break;
